...
export function AppShell({children}:{children:React.ReactNode}){
  ...
  return <div className="shell">
    <aside className="sidebar">
      <div className="brand"><FlaskConical size={24}/><span>BET<b>LAB</b></span><small>CORE 0.1</small></div>
      <nav>{items.map(([href,label,Icon])=>
        <Link key={href} href={href} className={pathname===href?"nav-link active":"nav-link"}>
          <Icon size={19}/><span>{label}</span>
        </Link>)}
      </nav>
      <div className="sync-card"><span className="sync-dot"/><div><strong>Mode local</strong><small>Sync Supabase à connecter</small></div></div>
    </aside>
